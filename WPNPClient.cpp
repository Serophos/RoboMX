...
void CWPNPClient::SetCallBackProck(RECVMESSAGEPROC *pProc, DWORD dwParam)
{
...
int CWPNPClient::Connect(PARENTNODEINFO *pNodeInfo, LPCTSTR lpszUserName, WORD wLineType, WORD wTransferPort)
{
...
BOOL CWPNPClient::LoginParent(PARENTNODEINFO *pNodeInfo, LPCTSTR lpszBaseName, CString &strWPNUserName, WORD wLineType, WORD wTransferPort)
{
	...
	*(WORD *)(buffer + 2) = (WORD)nNameLen + 5;
	...
	if((*(WORD*)buffer != 0x0460) || (*(WORD*)(buffer + 2) != 2)) return FALSE;
...
BOOL CWPNPClient::ProcWPNPMessage(WORD wType, WORD wLen, char *buffer)
{
	...
	if(wType == 0x238E){
		...
		TRACE("WPNP Recv: wType=%X len=%d data=%s\n", buffer);
	}
	return TRUE;
}
...
BOOL CWPNPClient::SendMessage(WORD wType, WORD wLen, char *pData)
{
